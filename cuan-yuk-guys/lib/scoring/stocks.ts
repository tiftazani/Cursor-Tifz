...
export function prepareStock(meta: StockMeta, bars: Bar[], ihsg: Bar[], extras?: Partial<Raw>): Raw {
  ...
  return {
    ...
    price: extras?.price ?? last.close,
    changePct: extras?.changePct ?? pctChange(prev.close, last.close),
    open: extras?.open ?? last.open,
    high: extras?.high ?? last.high,
    low: extras?.low ?? last.low,
    volume: extras?.volume ?? last.volume,
    week52High,
    week52Low,
    pe,
    pb,
