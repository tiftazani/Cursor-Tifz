...
Workflow `cuciin-health.yml` memeriksa Worker serta D1 setiap 15 menit. Workflow `cuciin-backup.yml` dijalankan manual untuk menguji export, enkripsi AES-256/PBKDF2 600.000 iterasi, checksum, dekripsi, impor SQLite, dan integrity check. Verifikator masih dapat membuka backup lama dengan iterasi PBKDF2 bawaan, lalu meminta backup itu dibuat ulang dengan format terbaru. Repository publik tidak mengunggah hasil backup sebagai artifact. Simpan backup terjadwal ke tujuan privat yang dikuasai Owner, misalnya bucket R2 privat atau repository backup privat, dengan retensi serta uji pemulihan terpisah. Prosedur rollout dan restore ada di `../OPERATIONS_RUNBOOK.md`.
