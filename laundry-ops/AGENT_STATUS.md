...
3. **`CuciinStore.branch()` melempar `NoSuchElementException`** karena memakai `first()` dan `first { }`. Katalog cabang bisa kosong sesaat atau memuat id yang belum tersinkron, dan ada 21 pemanggil di layar. Sekarang selalu mengembalikan nilai dengan cabang pengganti. Tiga tempat lain ikut diperbaiki: dua di `MasterScreens` dan `nextNotaId`.
